...
| 🔑 **Доступы** | Гибкие права: области (свои/склад) на просмотр/создание/редактирование/удаление по каждому складу + разделы меню и режимы приложения (чат/СРМ/прайс/задачи). Применяются реалтайм (SSE), раздаёт админ |
...
| ✅ **Задачи** | Тудулист: списки, сроки со временем, метки, подзадачи, ответственные; задача может быть привязана к заказу — тогда её видит каждый, кому виден заказ |
