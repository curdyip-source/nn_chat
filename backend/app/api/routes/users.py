...
from app.schemas.users import BootstrapFirstAdminPayload, UserCreatePayload, UserPermissionsPayload, UserProfileUpdatePayload, UserUpdatePayload
from app.services.profile_photos import upload_profile_photo
from app.services.users import UserService, bootstrap_first_user, create_user, delete_user, list_chat_participants, list_users, set_user_permissions, update_user, update_user_profile
...
def set_user_permissions_route(user_id: int, payload: UserPermissionsPayload, current_user: dict = Depends(require_admin), db: Session = Depends(get_db)) -> dict:
    return {"item": set_user_permissions(db, user_id, payload, current_user)}
