...
def register_user_device(db: Session, payload: UserDeviceRegisterPayload, current_user: dict) -> dict:
    row = UserDeviceRepository(db).register(
        user_id=current_user["user_id"],
        token=payload.user_device_token,
        platform=payload.user_device_platform,
        environment=payload.user_device_environment,
    )
    return {
        "user_device_id": row.user_device_id,
        "user_device_platform": row.user_device_platform,
        "user_device_environment": row.user_device_environment,
