...
class ProductRegistrationService:
    ...
    def update_product_registration(self, product_registration_id: int, payload: ProductRegistrationUpdatePayload, current_user: dict) -> dict:
        ...
        if payload.product_registration_status_id:
            status_id = get_status_or_404(self.db, payload.product_registration_status_id, expected_type="product_registration").status_id
        default_currency = get_default_currency_or_400(self.db)
        items = []
        for item in payload.items:
            product_row, article, name, cost = resolve_product_snapshot(
        ...
        row = self.repository.update_with_items(
            row,
            {
                "product_registration_establishment_id": payload.product_registration_establishment_id,
                "product_registration_supplier": payload.product_registration_supplier.strip(),
                "product_registration_status_id": status_id,
            },
            items,
        )
...
def list_product_registrations(db: Session, current_user: dict, *, page: int = 1, page_size: int = 20) -> dict:
    return ProductRegistrationService(db).list_product_registrations(current_user, page=page, page_size=page_size)


def get_product_registration(db: Session, product_registration_id: int, current_user: dict) -> dict:
    return serialize_product_registration(ProductRegistrationService(db).get_accessible_product_registration_or_404(product_registration_id, current_user))
...
def update_product_registration(db: Session, product_registration_id: int, payload: ProductRegistrationUpdatePayload, current_user: dict) -> dict:
    return ProductRegistrationService(db).update_product_registration(product_registration_id, payload, current_user)
