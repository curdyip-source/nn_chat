...
class UserService:
    ...
    def set_permissions(self, user_id: int, payload: UserPermissionsPayload, actor_user: dict) -> dict:
        # Права по складам (только админ). scope-значения валидирует схема; проверяем
        # существование складов. Полный набор: отсутствие склада = нет доступа.
        user = self.get_user_by_id_or_404(user_id)
        settings: list[dict] = []
        for entry in payload.establishments:
            get_establishment_or_404(self.db, entry.establishment_id)
            settings.append({
                "establishment_id": entry.establishment_id,
                "view_scope": entry.view_scope,
                "can_create": entry.can_create,
                "edit_scope": entry.edit_scope,
                "delete_scope": entry.delete_scope,
            })
        UserEstablishmentRoleRepository(self.db).replace_for_user(user_id, settings)
...
def set_user_permissions(db: Session, user_id: int, payload: UserPermissionsPayload, actor_user: dict) -> dict:
    return UserService(db).set_permissions(user_id, payload, actor_user)
